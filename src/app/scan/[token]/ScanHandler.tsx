...
export default function ScanHandler({ event, userId }: ScanHandlerProps) {
  ...
  // Record attendance
  const recordAttendance = useCallback(async (lat: number, lng: number, accuracy: number) => {
    try {
  ...
  const handleLocationAndAttendance = useCallback(async (position: GeolocationPosition) => {
    ...
    if (!isWithinGeofence) {
      setError(`You are ${(distance * 1000).toFixed(0)} meters away from the event location. Must be within ${event.radius_meters} meters.`)
      setLoading(false)
      return
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-lg shadow-md p-8 max-w-md text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Processing Attendance</h2>
          <p className="text-gray-600 mt-2">Verifying your location...</p>
          <p className="text-gray-400 text-xs mt-4">Please allow location access when prompted</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white rounded-lg shadow-md p-8 max-w-md text-center">
          ...
          <p className="text-gray-600">You have successfully checked in to {event.title}</p>
