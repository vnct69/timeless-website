...
export default async function NewEventPage() {
	...
	async function createEvent(formData: FormData) {
		...
		const radiusMeters =
			parseInt(formData.get("radiusMeters") as string) || 100;
		const expiryMinutes =
			parseInt(formData.get("expiryMinutes") as string) || 30;

		if (!title || !latitude || !longitude) {
			redirect(
				"/admin/events/new?error=Title, latitude, and longitude are required",
			);
		}
		...
		<div className="max-w-3xl mx-auto p-6">
			<div className="mb-6">
				...
				<p className="text-gray-600">
					Set up an event and generate a QR code for attendance tracking
				</p>
			</div>
			...
			>
				...
				<div className="border-t pt-6">
					...
					<div>
						...
						/>
						<p className="text-xs text-gray-500 mt-1">
							QR code will expire after this time (1-1440 minutes)
						</p>
					</div>
				</div>
			...
			<div className="mt-6 bg-blue-50 border border-blue-200 rounded-md p-4">
				<p className="text-sm text-blue-800">
					💡 <strong>Tip:</strong> To get the latitude and longitude of a
					location, use Google Maps (right-click → &quot;What&apos;s
					here?&quot;) or any geocoding service.
				</p>
			</div>
		</div>
	);
