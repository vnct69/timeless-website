...
export default async function EditEventPage({ params }: EditEventPageProps) {
  ...
  // Server action to update the event
  async function updateEvent(formData: FormData) {
    ...
    if (!title || !latitude || !longitude) {
      redirect(`/admin/events/${id}/edit?error=Title, latitude, and longitude are required`)
    }
    
    // Update QR expiry if changed
    const currentExpiry = new Date(event.qr_code_expiry)
    const newExpiry = new Date(Date.now() + expiryMinutes * 60000)
    
    // Only update expiry if the event is being reactivated or if it's a new expiry
    const shouldUpdateExpiry = isActive !== event.is_active || expiryMinutes !== event.radius_meters
    ...
    if (shouldUpdateExpiry && isActive) {
    ...
    if (updateError) {
      ...
      redirect(`/admin/events/${id}/edit?error=${encodeURIComponent(updateError.message)}`)
    }
    
    redirect(`/admin/events/${id}?success=Event updated successfully!`)
  }
