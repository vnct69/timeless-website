...
// 			if (!isWithinGeofence) {
// 				setError(
// 					`You are ${(distance * 1000).toFixed(0)} meters away. Must be within ${event.radius_meters} meters.`,
// 				);
// 				setSubmitting(false);
// 				return;
// 			}
...
// 						💡 <strong>Tip:</strong> The event code is the last part of the QR
// 						code URL. For example:{" "}
...
export default function DashboardScanClient() {
	...
	const handleScanSuccess = async (decodedText: string) => {
		...
		setEventCode(token);
		await handleRecordAttendance(token);
	};

	const handleRecordAttendance = async (code: string) => {
		if (!code.trim()) {
			setError("Please enter an event code");
			return;
		}
	...
	const handleManualSubmit = async (e: React.FormEvent) => {
		e.preventDefault();
		await handleRecordAttendance(eventCode);
	};
		...
		<div className="min-h-screen bg-gray-50 p-4">
			<div className="max-w-2xl mx-auto">
				<div className="mb-6">
					...
					<p className="text-gray-600">Scan the QR code or enter the event code manually</p>
				</div>
						...
						<div>
							<div className="bg-green-50 border border-green-200 p-4 rounded-lg mb-6">
								<p className="text-green-800 text-sm flex items-center gap-2">
									<span>✅</span> Location detected: {location.lat.toFixed(6)}, {location.lng.toFixed(6)}
								</p>
							</div>
							...
							<div className="mb-6">
								...
								<p className="text-xs text-gray-500 mt-2 text-center">
									{showScanner ? 'Point your camera at the QR code' : 'Use your phone camera to scan the QR code'}
								</p>
							</div>
